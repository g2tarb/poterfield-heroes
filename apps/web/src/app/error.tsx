  ...
  return (
    <main className="grid min-h-svh place-items-center bg-[var(--color-bg-base)] px-6 py-12">
      <div className="w-full max-w-lg text-center">
        ...
        <pre className="mt-6 max-h-40 overflow-auto rounded border border-[var(--color-border)] bg-[var(--color-bg-elev)] p-3 text-left font-mono text-[11px] text-[var(--color-fg-muted)]">
          {error.message}
          {error.digest ? `\n\ndigest: ${error.digest}` : ""}
        </pre>
        <div className="mt-6 flex flex-wrap justify-center gap-3">
          ...
          <a
            href="/"
            className="inline-flex min-h-[40px] items-center border border-[var(--color-border)] px-4 text-xs font-mono uppercase tracking-widest text-[var(--color-fg-muted)] transition hover:border-[var(--color-fg-primary)] hover:text-[var(--color-fg-primary)]"
          >
            ← atelier
          </a>
        </div>
